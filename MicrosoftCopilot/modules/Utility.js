    ...
    todos.forEach((todo, index) => {
        ...
        todoItem.querySelector('.delete-btn').addEventListener('click', () => {
            if (index !== -1) {
                ...
                // If the deleted to-do was active, set the first to-do in the list to active
...
export function setupEventListener() {
    ...
    document.getElementById('addTodoBtn').addEventListener('click', () => {
        ...
        const newTodo = new Entry(
            newTodoInput.value,
            false,
            true, // Set the new to-do to active
            []
        );
        todos.push(newTodo);
        localStorage.setItem('microsoftCopilot', JSON.stringify(todos));
    ...
    document.getElementById('addTaskBtn').addEventListener('click', () => {
        ...
        // Find the active to-do
        const activeTodo = todos.find(todo => todo.active);

        if (activeTodo) {
            // Append the new task to the entries of the active to-do
